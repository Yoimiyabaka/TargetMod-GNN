...
def test(model, graph, test_mask, device,batch, p_x_all, p_edge_all):
    ...
    for step in tqdm(range(valid_steps)):
        if step == valid_steps-1:
            valid_node_id = test_mask[step*batch_size:]
        else:
            valid_node_id = test_mask[step*batch_size : step*batch_size + batch_size]

        
        output = model(batch, p_x_all, p_edge_all, graph.edge_index,valid_node_id)
